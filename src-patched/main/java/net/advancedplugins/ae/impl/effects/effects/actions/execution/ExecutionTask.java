   ...
   public void init() {
      if (!this.soulboundOnly || !this.actionExecution.getAllEffects().stream().noneMatch(s -> s.equalsIgnoreCase("KEEP_ON_DEATH"))) {
         ...
         for (String effect : this.ability.getEffects()) {
            ...
            final LinkedList<LivingEntity> targetEntities = new LinkedList<>();
            ...
            try {
               if (loc != null) {
                  if (advancedEffect.executeEffect(this, loc, arguments)) {
                     ...
                     this.reportIssue(effect, "activating entity-based effect without a valid target (e.g. @Attacker)", this.ability.getName());
                  }
